...
public class PatientController {
    ...
    ResponseEntity<AppointmentDTO> createAppointment(@AuthenticationPrincipal final User user, AppointmentDTO appointment){

        Doctor doctor = doctorService.getDoctorById(appointment.getDoctorId());
        if(doctor == null)
            return new ResponseEntity<>(null, HttpStatus.CONFLICT);

        AppointmentDTO appointmentDTO = appointmentService.createAppointment(user, doctor.getUser(), appointment.getDateOfAppointment(),
    ...
    ResponseEntity<List<AppointmentDTO>> getAppointments(@AuthenticationPrincipal final User user){
        ...
        List<AppointmentDTO> appointments = appointmentService.getListOfPatientsAppointments(user);
    ...
    ResponseEntity<Appointment> getAppointment(@AuthenticationPrincipal final User user,  @PathVariable("id") final Long id){
        ...
        Appointment appointment = appointmentService.getAppointmentOfPatientById(user, id);
        if(appointment == null)
            return new ResponseEntity<>(null, HttpStatus.CONFLICT);
    ...
    ResponseEntity<AppointmentDTO> cancelAppointment(@AuthenticationPrincipal final User user,  @PathVariable("id") final Long id){
        ...
        AppointmentDTO appointmentDTO = appointmentService.cancelAppointment(user, id);
